...
public class PremierLeagueDAO {
	...
	public List<Player> getVertices(Map<Integer, Player> idMap, Integer matchID){
	...
	public List<Adiacenza> getAdiacenze(Map<Integer, Player> idMap, Integer matchID) {
		String sql = "SELECT distinct a1.PlayerID as p1, a2.PlayerID as p2, "
				+ " (((a1.TotalSuccessfulPassesAll+ a1.Assists)/ a1.TimePlayed ) - ((a2.TotalSuccessfulPassesAll+ a2.Assists)/ a1.TimePlayed )) AS peso "
		...
		try {
			...
			while (res.next()) {
				...
				if (idMap.containsKey(res.getInt("p1")) && idMap.containsKey(res.getInt("p2"))) {
					Adiacenza a = new Adiacenza(idMap.get(res.getInt("p1")), idMap.get(res.getInt("p2")),
							res.getDouble("peso"));
					result.add(a);
				}
			}
